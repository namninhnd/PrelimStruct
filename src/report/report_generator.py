...
<!-- ============================================
     PAGE 2: FEM DESIGN CHECKS
     ============================================ -->
<div class="page" id="page-design-checks">

    <h2 class="section-title">
        <span class="icon">{{ icons.ruler | safe }}</span>
        FEM Design Checks (HK Code 2013)
    </h2>

    {% if fem_data and fem_data.code_compliance %}
    <div class="fem-results-section" style="margin-bottom:var(--spacing-xl);">
        <h3 style="font-size:1rem;font-weight:700;color:var(--primary);margin-bottom:var(--spacing-md);">
            {{ icons.check | safe }} Code Compliance Summary
        </h3>
        <div class="status-grid" style="grid-template-columns:repeat(3,1fr);">
            {% for item in fem_data.code_compliance[:3] %}
            <div class="status-card {{ 'pass' if item.status == 'OK' else 'warn' if item.status == 'WARNING' else 'fail' }}">
                <div class="element-name">{{ item.check }}</div>
                <div class="utilization">{{ item.status }}</div>
            </div>
            {% endfor %}
        </div>
    </div>
    ...
    <!-- Per-Type Top-3 FEM Design Checks -->
    {% if fem_data and fem_data.top3_by_type %}
    {% for type_name, type_items in fem_data.top3_by_type.items() %}
    <div class="calc-section" style="margin-bottom:var(--spacing-lg);">
        <h3 style="font-size:0.95rem;font-weight:700;color:var(--primary);margin-bottom:var(--spacing-sm);">
            Top-3 {{ type_name }}
        </h3>
        <table style="width:100%;border-collapse:collapse;font-size:0.85rem;">
            <thead>
                <tr style="background:var(--bg-alt);border-bottom:2px solid var(--primary);">
                    <th style="padding:6px 8px;text-align:left;">Rank</th>
                    <th style="padding:6px 8px;text-align:left;">Element</th>
                    <th style="padding:6px 8px;text-align:left;">Key Metric</th>
            ...
            {% for item in type_items %}
                <tr style="border-bottom:1px solid #eee;">
                    ...
                    <td style="padding:6px 8px;font-weight:600;">{{ item.element_id }}</td>
                    <td style="padding:6px 8px;">{{ item.key_metric }}</td>
                    <td style="padding:6px 8px;text-align:right;font-weight:700;">{{ "%.2f" | format(item.score) }}</td>
                    <td style="padding:6px 8px;text-align:center;">
                        {% if item.score < 0.8 %}&#x1F7E2;{% elif item.score < 1.0 %}&#x1F7E1;{% else %}&#x1F534;{% endif %}
                    </td>
                </tr>
            {% endfor %}
            </tbody>
    ...
    <div class="calc-section">
        <p style="color:#64748B;">No FEM element forces available. Run analysis to populate design checks.</p>
    </div>
    {% endif %}
...
<div class="page" id="page-fem">
    ...
    <!-- FEM vs Hand-Calc Comparison -->
    <h3 class="section-title" style="font-size: 1rem;">
        <span class="icon">{{ icons.ruler | safe }}</span>
        FEM vs Hand-Calc Design Comparison
    </h3>
    ...
    <div class="fem-comparison-grid">
        <div class="fem-card">
            ...
            <div class="fem-value-row">
                <span class="label">Max Moment (Hand-Calc)</span>
        ...
        <div class="fem-card">
            ...
            <div class="fem-value-row">
                <span class="label">Max Axial (Hand-Calc)</span>
        ...
        <div class="fem-card">
            ...
            <div class="fem-value-row">
                <span class="label">Drift (Hand-Calc)</span>
...
class ReportGenerator:
    ...
    def _build_fem_data(
        ...
        data = {
            ...
            'top3_by_type': {},
            'design_check_warnings': [],
        }

        # Build per-type Top-3 from FEM model if available
        try:
            from src.fem.design_check_summary import compute_design_checks_summary

            model = getattr(self.project, '_fem_model', None)
            results_by_case = getattr(self.project, '_fem_results_by_case', None)
            selected_names = getattr(self.project, '_selected_combinations', None)
            if model and results_by_case:
                summary = compute_design_checks_summary(
                    project=self.project,
                    model=model,
                    results_by_case=results_by_case,
                    selected_combination_names=selected_names,
                    top_n=3,
                )
                data['top3_by_type'] = summary.top3_by_type
                data['design_check_warnings'] = summary.warnings
        except Exception:
            pass  # Graceful fallback if design checks not available
