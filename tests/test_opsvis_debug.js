...
async function runTest() {
    ...
    try {
        ...
        if (await selectbox.count() > 0) {
            ...
            console.log('7. Selecting M-major force type...');
            const majorOption = page.locator('li:has-text("M-major")');
            if (await majorOption.count() > 0) {
                await majorOption.first().click();
                console.log('   Selected M-major option');
            } else {
            ...
            const radioMy = page.locator('label:has-text("M-major")');
            if (await radioMy.count() > 0) {
                console.log('   Found M-major radio button, clicking...');
