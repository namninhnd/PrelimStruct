...
async function testForceDiagramParabolicCurves() {
    ...
    try {
        ...
        console.log('\n4. Selecting M-major force type from radio buttons...');
        ...
        const mMajorRadio = page.locator('label:has-text("M-major (Strong Axis)")');
        if (await mMajorRadio.count() > 0) {
            await mMajorRadio.scrollIntoViewIfNeeded();
            await mMajorRadio.click();
            await page.waitForTimeout(2000);
            console.log('   Selected M-major (Strong Axis)');
        } else {
            console.log('   M-major radio not found, trying alternatives...');
            
            const anyForceRadio = page.locator('label:has-text("M-minor"), label:has-text("N (Axial)"), label:has-text("V-major")');
